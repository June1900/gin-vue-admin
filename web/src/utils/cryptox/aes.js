// cryptox/aes.js - AES-256-CBC 加解密工具
 ...
 * @param {string|Uint8Array} plainText 已序列化的明文(JSON 字符串或普通文本)
 * @param {ArrayBuffer|Uint8Array} keyBuf 32 字节 key
 */
export async function encryptAes(plainText, keyBuf) {
  ...
  const iv = crypto.getRandomValues(new Uint8Array(AES_BLOCK_SIZE))
 ...
 */
export async function decryptAes(cipherB64, keyBuf) {
  const keyBytes = toUint8(keyBuf)
  if (keyBytes.length !== AES_KEY_LENGTH) {
    throw new Error(`AES key must be ${AES_KEY_LENGTH} bytes, got ${keyBytes.length}`)
  }
