 ...
 */
export async function importPrivateKey(pemOrBase64) {
  const der = pemOrDer(pemOrBase64)
  return crypto.subtle.importKey(
    'pkcs8',
    der,
 ...
 */
export async function encryptRsa(plainBuf, publicKey) {
  const out = await crypto.subtle.encrypt({ name: 'RSA-OAEP' }, publicKey, plainBuf)
 ...
 */
export async function decryptRsa(cipherB64, privateKey) {
  const buf = base64ToArrayBuffer(cipherB64)
  return crypto.subtle.decrypt({ name: 'RSA-OAEP' }, privateKey, buf)
}
...
function pemOrDer(input) {
  ...
  try {
    ...
    throw new Error(`base64 decode failed: ${e.message}; input head="${input.slice(0, 60)}..."`)
  }
